#!/bin/bash
# Router launcher for SLO config evaluation
# Usage: launch_router.sh <scheduler_config_path>

CONFIG_FILE=${1:-/sgl-workspace/sglang/slo/scheduler_config_slo_aware_ttft.json}

cd /sgl-workspace/sglang/sgl-router && pip install . > /dev/null 2>&1
...
python -m sglang_router.launch_router \
  ...
  --worker-urls \
    ...
    http://0.0.0.0:31007 \
    http://0.0.0.0:31008 \
  --scheduler-config-file $CONFIG_FILE
